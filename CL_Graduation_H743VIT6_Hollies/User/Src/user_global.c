...
static INA228_State INA228_0_state = {
    ...
    .adcConfigRegister = (INA228_adc_config_register_mode_Continuousshuntandbusvoltage |
    ...
    .shuntCalRegister = (uint16_t)(13107.2f * 1e6 * 8.f / 524288.f * 0.02f),
...
float loopI = 1.f;      // 电流环电流
float M = 1.f;          // 调制比

three_Phase_Signal_V *signal_V;
three_Phase_Signal_I *signal_I;
single_Phase_Signal_V *signal_V_single;
single_Phase_Signal_I *signal_I_single;

volatile enum run_state runState = STOP;           // 运行状态
volatile enum input_output_mode inputMode = INIT;  // 输入模式
volatile enum input_output_mode outputMode = INIT; // 输出模式
