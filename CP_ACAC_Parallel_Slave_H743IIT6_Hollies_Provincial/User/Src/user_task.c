 ...
 * @LastEditTime : 2024-08-01 14:25:48
 * @FilePath     : \CP_ACAC_Parallel_Slave_H743IIT6_Hollies_Provincial\User\Src\user_task.c
...
void StartOledDisplay(void *argument)
{
    ...
    for (;;)
    {
        snprintf(str, sizeof(str), "Uo:%5.2f", allSignal->voltageOutRms);
        OLED_ShowString(0, 0, (uint8_t *)str, 12);

        snprintf(str, sizeof(str), "USet:%4.1f", UoValueSet);
        OLED_ShowString(0, 12, (uint8_t *)str, 12);

        snprintf(str, sizeof(str), "Duty:%3.2f", allSignal->pid->out);
...
void StartMainControl(void *argument)
{
    ...
    UNUSED(argument);
    pid_Init(allSignal->pid, 0.5f, 0.3f, 0.f, 0.95f, 0.80f);
