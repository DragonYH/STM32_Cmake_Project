 ...
 * @LastEditTime : 2024-07-29 15:39:05
 * @FilePath     : \CP_ACAC_Parallel_H743IIT6_Hollies_Provincial\User\Src\pid.c
 * @Description  : PID控制器
 *
 ...
 * @param min 输出最小值
 */
void pid_Init(PID_t *pid, float kp, float ki, float kd, float max, float min) // PID参数初始化
{
	pid->kp = kp;
	pid->ki = ki;
	pid->kd = kd;
 ...
 * @note result代表增量输出
 */
void pid(PID_t *pid, float target, float sample)
{
