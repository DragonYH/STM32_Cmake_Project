 ...
 * @LastEditTime : 2024-08-01 19:41:52
 * @FilePath     : \CP_ACAC_Parallel_H743IIT6_Hollies_Provincial\User\Src\user_task.c
...
static float IoRatioSet = 1.f;  /* 输出电流比例设定值 */
...
void StartOledDisplay(void *argument)
{
    ...
    for (;;)
    {
        snprintf(str, sizeof(str), "Uo:%5.2f USet:%4.1f", allSignal->voltageOutRms, UoValueSet);
        OLED_ShowString(0, 0, (uint8_t *)str, 12);

        snprintf(str, sizeof(str), "Io:%5.2f", allSignal->currentOutRms);
        ...
        snprintf(str, sizeof(str), "IRat:%4.2f RSet:%3.1f", Iratio, IoRatioSet);
        OLED_ShowString(0, 24, (uint8_t *)str, 12);

        snprintf(str, sizeof(str), "IAll:%4.2f Duty:%4.2f", allSignal->currentOutAllRms, allSignal->pid->out);
...
void pwmControl(void)
{
    ...
    __HAL_TIM_SET_COMPARE(&htim8, TIM_CHANNEL_2, allSignal->pid->out * TIM_PERIOD);
}
...
void StartMainControl(void *argument)
{
    ...
    for (;;)
    {
        if (deviceState == RUN)
        {
            ...
            {
                allSignal->pid->kp = 0.005f;
                allSignal->pid->ki = 0.001f;
                allSignal->pid->Max = 0.92f;
                allSignal->pid->Min = 0.8f;
...
void StartStateControl(void *argument)
{
    ...
    for (;;)
    {
        ...
        {
            osDelay(1000);
            if (allSignal->voltageOutRms > 25.f)
            {
                mode = CURRENT_MODE;
                deviceState = RUN;
            }
