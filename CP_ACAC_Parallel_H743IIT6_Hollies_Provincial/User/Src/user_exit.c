 ...
 * @LastEditTime : 2024-08-01 19:37:23
...
static void getVoltageCurrent(void)
{
    ...
    allSignal->currentOut = adcValue[4] * 4.f;          /* 输出电流 */
...
static void calcEffectiveValue(void)
{
    ...
    if (++cnt == 400)
    {
        ...
        allSignal->currentOutRms = 0.2641f * allSignal->currentOutRms - 0.0293f;
        allSignal->voltageOutRms += 0.3f;
    }
}
